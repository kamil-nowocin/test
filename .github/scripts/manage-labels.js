...
const resultLabel = process.env.build_result === 'success' ? 'build-success'
    : 'build-failure';
const removeLabel = process.env.build_result === 'success' ? 'build-failure'
    : 'build-success';
...
async function manageLabels(github, context, labels, resultLabel, removeLabel) {
  ...
  try {
    ...
    console.log(
        `Failed to remove label: ${removeLabel}. It might not have been added yet.`);
  }
  await github.rest.issues.addLabels({
    owner, repo, issue_number, labels: [resultLabel]
  });
}
